  ...
  return (
    <div className="rounded-lg overflow-hidden shadow-lg bg-white relative">
      ...
      <img
        ...
        src={formatProfileImageUrl(image)}
        alt={`${name} profile`}
      />
      <div className="p-4">
        ...
        <div className="mt-4 flex justify-between">
          <button
            onClick={() =>
              navigate("/doctor/profile", { state: { doctor: rawDoctorData } })
            }
          ...
          <button
            onClick={() =>
              navigate("/patient/booking", {
                state: { doctor: { id: rawDoctorData.doctor_id } },
              })
            }
...
const Favourites = () => {
  ...
  const fetchFavouriteDoctors = async () => {
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/patient/favorite-doctors?page=1&limit=3`,
        {
          headers: {
      ...
      const mapped = data.map((doc) => ({
        name: `${doc.prefix || ""} ${doc.f_name || ""} ${
          doc.l_name || ""
        }`.trim(),
        specialties:
          doc.specialization?.map((s) => s.name).join(", ") || "Unknown",
        location:
          [
            doc.address_line1,
            doc.address_line2,
        ...
        feeRange:
          doc.consultation_fee_type === "paid"
            ? `AUD ${doc.consultation_fee || 0}`
            : "Free",
        availableDate: doc.next_available || "Not Available",
        image: formatProfileImageUrl(doc.profile_image), // Use default avatar if no image
        rawDoctorData: doc,
      }));
  ...
  const removeFavorite = async (doctorId) => {
    try {
      ...
      setDoctors((prev) =>
        prev.filter((doc) => doc.rawDoctorData.doctor_id !== doctorId)
      );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {doctors.map((doctor, index) => (
              <DoctorCard
                ...
                isRemoving={
                  isRemovingFavorite === doctor.rawDoctorData.doctor_id
                }
              />
            ))}
          </div>
