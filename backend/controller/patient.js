  ...
  updateProfile: async (req, res) => {
    ...
    try {
      ...
      const parseIfJsonString = (value) => {
        if (typeof value === "string") {
          try {
            ...
            return value; // If not a valid JSON string, return as is
          }
        }
        return value;
      };
      ...
      const profileFields = {
        ...
        similar_problem_before:
          had_similar_problem === "true" || had_similar_problem === true,
        pain_description: Array.isArray(parseIfJsonString(pain_description))
          ? JSON.stringify(parseIfJsonString(pain_description))
          : null,
        symptoms: Array.isArray(parseIfJsonString(symptoms))
          ? JSON.stringify(parseIfJsonString(symptoms))
          : null,
        problem_status,
        pain_triggers: Array.isArray(parseIfJsonString(pain_triggers))
          ? JSON.stringify(parseIfJsonString(pain_triggers))
          : null,
        ...
        taken_cortisone:
          ever_taken_cortisone === "true" || ever_taken_cortisone === true,
        pregnancy_status,
        high_blood_pressure: parseIfJsonString(medical_conditions)?.includes?.(
          "high_blood_pressure"
        ),
        heart_problems:
          parseIfJsonString(medical_conditions)?.includes?.("heart_problems"),
        diabetes: parseIfJsonString(medical_conditions)?.includes?.("diabetes"),
        cancer: parseIfJsonString(medical_conditions)?.includes?.("cancer"),
      };
