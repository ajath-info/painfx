...
updateProfile: async (req, res) => {
  ...
  try {
    ...
    if (f_name !== undefined && f_name.trim() === "") {
      return apiResponse(res, { error: true, code: 400, message: "First name cannot be empty" });
    }
    if (l_name !== undefined && l_name.trim() === "") {
      return apiResponse(res, { error: true, code: 400, message: "Last name cannot be empty" });
    }
    if (prefix !== undefined && !["Mr", "Mrs", "Ms", "Dr"].includes(prefix)) {
      return apiResponse(res, { error: true, code: 400, message: "Invalid prefix" });
    }
    ...
    if (f_name !== undefined || l_name !== undefined || prefix !== undefined) {
      updatedData.full_name = `${prefix || admin.prefix} ${f_name || admin.f_name} ${l_name || admin.l_name}`;
    }
